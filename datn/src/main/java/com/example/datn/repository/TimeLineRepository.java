...
public interface TimeLineRepository extends JpaRepository<TimeLine, Long> {
    ...
    @Query("select count(distinct t.hoaDon.hoaDonId) from TimeLine t where t.ngayTao = :ngayTao and t.hoaDon.trangThai = :trangThai")
    Long countHoaDonByNgayTaoAndTrangThai(@Param("ngayTao") LocalDate ngayTao, @Param("trangThai") int trangThai);

    @Query("select count(distinct t.hoaDon.hoaDonId) from TimeLine t where year(t.ngayTao) = :year and month(t.ngayTao) = :month and t.hoaDon.trangThai = :trangThai")
    ...
    Double sumHoaDonByYearMonthDayAndTrangThai(@Param("year") int year,
